...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  try {
    ...
    } else throw `> *[❗معلومه❗]قم بالرد على صورة او فيديو او جيف لتحويله لملصق. ${usedPrefix + command}*`
  } catch (error) {
    console.log(error)
    try {
      let [packname, ...author] = args.join` `.split`|`
      ...
      if (args[0] && isUrl(args[0])) {
        ...
        throw `*> [❗معلومه❗]  مثال: ${usedPrefix}s https://telegra.ph/file/0dc687c61410765e98de2.jpg*`
      }
    } catch (error) {
      stiker = `> *[❗معلومة❗] تأكد من الرد على صورة او مقطع فيديو قصير لتحويله لملصق بنجاح*`
    }
  } finally {
    m.reply(stiker)
  }
